...
public class Client {
    ...
    Client(String host_, int port_, String settingsFilename) {
        host = host_;
        port = port_;
    ...
    public void runClients() {
        ...
        for (int i = 0; i < numConnections; i++) {
            requesters.add(new Thread(new Requester(host, port, i, logger, settings, cache)));
            requesters.get(i).start();
        }

        // Join the threads (after they might have stopped due to the server terminating)
        for (int i = 0; i < numConnections; i++) {
            try {
    ...
    public static void main(String[] args) {
        Client c = new Client("localhost", 1025, "client.properties");
        c.runClients();
    }
}